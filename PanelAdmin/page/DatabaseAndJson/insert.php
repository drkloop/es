...
} elseif ($Res_data->action == "deleteAver") {
    $funcs = $Res_data->func;
    switch ($funcs) {
        case 'sarasari':
            $query = "DELETE FROM global_advertise
                WHERE 	g_advertise_id = $Res_data->Id
                     ";
            try {
            ...
            $query = "DELETE FROM advertise
                WHERE advertise_id = $Res_data->Id
                     ";
            try {
                $delete = mysqli_query($database->getDb(), $query);
            } catch (Exception $e) {
...
}elseif ($Res_data->action == "pasokhEr") {
    $query = "UPDATE report_er SET
     	javab='" . $Res_data->text . "' ,visited='1' WHERE Id= '" . $Res_data->id . "'";
    try {
        $update = mysqli_query($database->getDb(), $query);
    } catch (Exception $e) {
        echo $e->errorMessage();
